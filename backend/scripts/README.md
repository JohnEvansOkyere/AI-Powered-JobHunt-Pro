...
| `apply_migrations.py` | Deployment migration runner. Validates and atomically applies tracked migrations from `019` onward using `DATABASE_URL`; `--check` performs CI-only file validation. |
